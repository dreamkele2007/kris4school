...
import {Button,Modal,Form,Input,notification,Popconfirm} from 'antd';
...
class ManualForm extends React.Component {
  state = {
    ...
    divStatus:this.props.divStatus,
      ...
      this.props.form.validateFields((err, values) => {
        if(!err){
          if(this.isNull(values.manufacturingPlantName)||this.isNull(values.manufacturingPlantAddress)||
            ...
            var staB=this.props.divStatus;
            var arr=this.props.selectedRowKeys;
            var val=this.props.selectedRows;
            val.splice(0, 1);
            staB.splice(0, staB.length);
            arr.splice(0,arr.length);
            var staA=['none']
            staB=staA;
            this.state.divStatus=staB;
            processor.updateData(this.props.paginationValue,this.props,domainDataId,submitId,values,name,this)
  ...
  //直接稽核通过
  updateAndCheckedData=()=>{
    const gtin=this.props.record.gtin;
    this.props.domainValue.gs1Gtin=gtin;
    this.props.domainValue.productdataStatus='1'
    processor.updateAndCheckedData(this.props.paginationValue, this.props.domainValue, this.props,this.props.name);
    //直接稽核通过重新查询已稽核数据
    processor.loadDataOne(null, this.props, this.props.submitId,"haveAudit");
    //修改已稽核和未稽核的数量
    var num1=this.props.notAuditedNumArr[0];
    var num2=this.props.haveAuditNumArr[0];
    //计算直接稽核后，稽核和未稽核的个数
    var notAudited= num1- 1;
    var haveAudit= num2+ 1;
  ...
  render() {
    ...
    return (
      <span>
         <Button size="small" type="primary" onClick={this.onRowClick}>修改</Button>
         &nbsp;&nbsp;
         <Popconfirm title="确定直接稽核通过吗?" onConfirm={this.updateAndCheckedData}>
           <Button size="small" type="primary" >直接稽核</Button>
         </Popconfirm>
            <Modal
            title="修改数据"
                     ...
                     <td style={{width:151,textAlign:'center'}}>
                       <FormItem>
                          {getFieldDecorator('imageLink',{
                            ...
                            <span>
                             {this.props.form.getFieldValue('imageLink')==null? null:<img width={150} height={120} src={this.props.form.getFieldValue('imageLink')}/>}
                            </span>
                          )}
                        </FormItem>
                     </td>
                     <td style={{width:151,textAlign:'center'}}><img width={150} height={120} src={this.props.record.imageLink}/></td>
                     &nbsp;&nbsp;&nbsp;
                     <td style={{width:151,textAlign:'center'}}>
...
ManualForm =Form.create()(ManualForm);
